...
import {readCacheData, saveCacheData} from "./cache";
...
const loadFromAlbum = async (albumId) => {
    ...
    const cached = readCacheData(`google_album_${albumId}`)
    if (cached) return cached;

    const data = await libraryApiSearch(parameters);
    const result = data.photos
    saveCacheData(`google_album_${albumId}`, result, 500)
    return result
}
...
async function authToken() {
    let cached = readCacheData('google_acccess_token')
    if (cached) return cached;
    ...
    if (result) {
        ...
        saveCacheData('google_acccess_token', result.access_token, result.expires_in)
