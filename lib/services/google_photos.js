...
const albumsByTitles = async (titles) => {
    let photos = await Promise.all(
        titles.map(t => {
            return albumByTitle(t)
        })
    )
    return photos.flat()
}
const albumByTitle = async (title) => {
...
const loadFromAlbumById = async (albumId) => {
    ...
    writeCacheData(cache_key, JSON.stringify(photos), 500)
    return photos
}
...
export {albumByTitle, albumsByTitles}
