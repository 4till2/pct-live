...
export default function Map({ waypoints }) {
  ...
  const [lng] = useState(-120.802145);
  const [lat] = useState(49.000181);
    ...
    <>
      ...
      <div className="map-wrap text-black">
        <div className="p-2 fixed top-4 ml-2 z-10 rounded-md bg-gray-300/80 text-xs text-center justify-center">
          <div>
            Click a <span className="font-bold">Waypoint</span>{" "}
...
export async function getServerSideProps() {
  ...
  let waypoints = await parseKML.toJson("data/garmin_pct.kml").then((e) => {
    return e.features
      .map((entry) => {
        return {
          title: "Waypoint",
